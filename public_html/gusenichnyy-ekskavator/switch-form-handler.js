// Скрипт для переключения между web3forms и PHP обработчиком - ГУСЕНИЧНЫЙ ЭКСКАВАТОР
...
if (mode === 'web3forms') {
    ...
    const web3formsContent = `// modal.js - WEB3FORMS ВЕРСИЯ - ГУСЕНИЧНЫЙ ЭКСКАВАТОР
    ...
    function showContactModal() {
        ...
        contactModalWindow.innerHTML = \`
            ...
            <form class="modal-form" id="formaGusenichnyexkavator" action="https://api.web3forms.com/submit" method="POST">
                ...
                <input type="text" name="name" placeholder="Ваше имя" required autocomplete="name">
                <input type="text" name="contact" placeholder="Телефон или e-mail" required autocomplete="tel">
                <input type="hidden" name="subject" value="Новая заявка с сайта ГУСЕНИЧНЫЙ ЭКСКАВАТОР (см. больше)">
        ...
        form.addEventListener('submit', function(e) {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    if (typeof ym === 'function') {
                        ym(103422283, 'reachGoal', 'formaGusenichnyexkavator');
                        console.log('Цель "Отправка формы из модального окна" отправлена в Яндекс Метрику (formaGusenichnyexkavator)');
                    }
                    form.style.display = 'none';
                    success.textContent = 'Заявка отправлена!';
                    success.style.display = 'block';
